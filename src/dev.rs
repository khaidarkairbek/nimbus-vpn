use crate::{
    crypto::{generate_public_key, generate_shared_key},
    tun::TunDevice,
};
...
use crate::error::{ClientError::*, CommError::*, LogicError::*, ServerError::*, SocketError::*};
            ...
            Device::Client {
                ...
                while bytes_written < data.len() {
                    bytes_written += socket
                        .send_to(&data[bytes_written..data.len()], *server_addr)
                        .map_err(|e| SocketSendToError(e.to_string()))?;
                }
                Ok(())
            }
            Device::Server {
        ...
        match self {
            ...
            | Device::Server {
                ...
                let (len, from_addr) = socket
                    ...
                    .map_err(|e| SocketReadError(e.to_string()))?;
                let msg = serde_json::from_slice::<Message>(&buffer[..len])
                    .map_err(|e| DeserialError(e.to_string()))?;
                Ok((from_addr, msg))
            }
        }
    }
        ...
        match self {
            Device::Client {
                ...
                let serialized = serde_json::to_string::<Message>(&request_msg)
                    .map_err(|e| SerialError(e.to_string()))?; //serialize the message to json

                client_socket
                    .send_to(serialized.as_bytes(), *server_addr)
                    .map_err(|e| SocketSendToError(e.to_string()))?;
                    ...
                    match available_ids.pop() {
                        Some(client_id) => {
                            ...
                            let shared_secret_key =
                                generate_shared_key(&client_public_key, private_key);
        ...
        match self {
            Device::Client {
                ...
                *default_gateway = if cfg!(target_os = "macos") {
                    let default_gw_output = process::Command::new("route")
                        .arg("-n")
                        .arg("get")
                        .arg("default")
                        .output()
                    ...
                    assert!(process::Command::new("route")
                        .arg("add")
                    ...
                    let default_gw_output = process::Command::new("ip")
                        .arg("route")
                        .arg("show")
                        .arg("default")
                        .output()
                        .unwrap();
                    assert!(default_gw_output.status.success());
                    let mut words = str::from_utf8(&default_gw_output.stdout)
                        .unwrap()
                        .split_whitespace();
