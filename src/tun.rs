...
use crate::error::{TunInitError::*, TunOperationError::*};
...
impl TunDevice {
            ...
            Some(tun_num) => {
                ...
                unsafe {
                    libc::memset(
                        ...
                        0,
                        mem::size_of::<CtlInfo>() as libc::size_t,
                    ); // Clear the kernel info by modifying the memory
                    let ctl_name = std::ffi::CString::new("com.apple.net.utun_control").unwrap();
                    libc::strncpy(
                        ctl_info.ctl_name.as_mut_ptr() as *mut libc::c_char,
                        ctl_name.as_ptr(),
                        96,
                    ); // Set the kernel control name
                };

                if unsafe { libc::ioctl(file.as_raw_fd(), libc::CTLIOCGINFO, &mut ctl_info) } == -1
                {
                    // Getting kernel control id, break if not succesfull
            ...
            Some(tun_num) => {
                ...
                unsafe {
                    libc::memset(
                        ...
                        0,
                        mem::size_of::<libc::ifreq>() as libc::size_t,
                    ); // Clear the kernel info by modifying the memory
                }
    ...
    pub fn up(&self, server_client_id: Option<u8>) {
        ...
        let mut status = process::Command::new("ifconfig")
            .arg(format!("utun{}", self.id))
            .arg(format!("10.20.20.{}", id))
            .arg("10.20.20.1")
            .status()
            .unwrap();
        ...
        status = process::Command::new("ifconfig")
            .arg(format!("utun{}", self.id))
            .arg("mtu")
            .arg(MTU)
            .arg("up")
    ...
    pub fn up(&self, server_client_id: Option<u8>) {
        ...
        let mut status = process::Command::new("ip")
            .arg("addr")
            .arg("add")
            .arg(format!("10.20.20.{}/24", id))
            .arg("dev")
            .arg(format!("tun{}", self.id))
            .status()
            .unwrap();
        assert!(status.success());

        // Setting MTU (Maximum Transmission Unit) value to 1380, which is a standard for VPN application
        status = process::Command::new("ip")
            .arg("link")
            .arg("set")
            .arg("dev")
            .arg(format!("tun{}", self.id))
    ...
    pub fn write(&mut self, buffer: &[u8]) -> Result<usize> {
        self.file
            .write(buffer)
            .map_err(|e| anyhow!(TunWriteError(e)))
    }
